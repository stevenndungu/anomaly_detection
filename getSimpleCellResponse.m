function [response, excitationresponse, responseParams] = getSimpleCellResponse(img,model,orientlist,inhibitionFactor,responseParams)
...
for i = 1:numel(orientlist)    
    ...
    excitationresponse(:,:,i) = getResponse(SimpleCellHashTable,rotmodel.simpleCell.excitation,weightsigma,model.params);
    ...
    if isfield(model.simpleCell,'inhibition')
        ...
        rotresponse = excitationresponse(:,:,i) - inhibitionFactor*inhibitionresponse;
        rotresponse = rotresponse .* (rotresponse > 0);
    else
        % If no inhibition, then we only take the excitatory response
        rotresponse = excitationresponse(:,:,i);
    end
...
for i = 1:ntuples
    ...
    sigma = round(simpleCell(2,i)*1000)/1000;
...
sigmaList = round(unique(sigmaList)*1000)/1000;
